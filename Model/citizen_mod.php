...
private function generateReferenceNumber() {
    ...
    public function getAvailablePriests($selectedDate, $startTime, $endTime) {
        ...
        WHERE 
            ...
            AND NOT EXISTS (
                -- Check if priest is already booked for any event on the selected date and time
                SELECT 1
                FROM (
                    SELECT pa.priest_id
                    FROM schedule s
                    LEFT JOIN baptismfill b ON s.schedule_id = b.schedule_id
                    LEFT JOIN confirmationfill cf ON s.schedule_id = cf.schedule_id
                    LEFT JOIN defuctomfill df ON s.schedule_id = df.schedule_id
                    LEFT JOIN marriagefill mf ON s.schedule_id = mf.schedule_id
                    LEFT JOIN announcement ann ON s.schedule_id = ann.schedule_id
                    LEFT JOIN req_form rf ON s.schedule_id = rf.schedule_id
                    LEFT JOIN mass_schedule ms ON s.schedule_id = ms.schedule_id
                    LEFT JOIN priest_approval pa 
                        ON pa.approval_id IN (b.approval_id, cf.approval_id, df.approval_id, mf.approval_id, ann.approval_id, rf.approval_id, ms.approval_id)
        ...
        // Bind the parameters
        $stmt->bind_param(
            "sss",  // 3 string parameters (selectedDate, startTime, endTime)
